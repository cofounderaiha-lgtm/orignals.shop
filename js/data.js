...
/* ============================================================
   DATA QUALITY LABELS — the 14 shops above are CATALOGUE SEED
   DATA, not real merchants: nobody is behind them, so an order
   placed on one cannot be accepted, packed or dispatched by a
   human. They exist so a first-time user sees a populated app.

   Real sellers arrive via cloudShopsRefresh() and carry
   `community: true` + an id like `my_xxxxxxxx`.

   Everything that measures the BUSINESS — analytics, GMV,
   recommendations, seller rankings, trust scores, financial
   reporting — must exclude demo entities, or the platform ends
   up reporting revenue that never existed. That is the whole
   point of this block: one flag, set in one place, so nothing
   downstream has to guess.
   ============================================================ */
DB.shops.forEach(s => { if (!s.community) { s.demo = true; s.dq = 'demo'; } });

/* is this shop a real, human-operated seller? */
function isRealShop(shop) { return !!(shop && shop.community && !shop.demo); }

/* data-quality label for anything derived from an order/shop:
   'real'  — a genuine transaction with a human seller behind it
   'demo'  — placed against seed catalogue data (never counts)
   'test'  — deliberately generated by admin/QA tooling          */
function dqOf(shop) { return isRealShop(shop) ? 'real' : 'demo'; }

/* keep only rows that may inform business metrics / models */
function realOnly(rows) { return (rows || []).filter(r => (r && (r.dq || 'real')) === 'real'); }
